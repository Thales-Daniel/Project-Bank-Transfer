...
export const validateUser = async (req: Request,_res: Response,next: NextFunction) => {
  ...
  if(/[A-Z]/.test(password)) throw new ErrorApp("the password must contain a uppercase letter", 400)
  if(/[0-9]/.test(password)) throw new ErrorApp("the password must contain a uppercase letter", 400)
